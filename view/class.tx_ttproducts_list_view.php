...
class tx_ttproducts_list_view {
	...
	public function &printView (
		...
		$globalMarkerArray['###FORM_MEMO###'] = htmlspecialchars($pibaseObj->pi_getPageLink($pid,'',$this->urlObj->getLinkParams($excludeList, array(), TRUE)));
...
// 		}
		...
		if ($allowedItems || $allowedItems == '0')	{
			...
			$allowedItemArray = $TYPO3_DB->fullQuoteArray($tempArray,$itemTable->getTablename());
...
/*					$catFrom = $categoryTable->getTableObj()->name.' '.$catAlias;
			...
			$begin_at = t3lib_div::intInRange(($begin_at >= $productsCount) ? ($productsCount >= $limit ? $productsCount - $limit : $productsCount) : $begin_at, 0);
...
/*							$markerArray = array_merge($productMarkerArray, $categoryMarkerArray, $markerArray);*/
		...
		if (count($itemArray))	{
			...
			$linkUrl = $pibaseObj->pi_getPageLink($pid,'',$this->urlObj->getLinkParams('',array(),TRUE));
			...
			if ($more)	{
				$next = ($begin_at+$limit > $productsCount) ? $productsCount-$this->config['limit'] : $begin_at+$limit;
			...
			if ($itemTable->getType() == 'dam' && is_object($relatedListView))	{
				...
				$subpartArray['###ITEM_CATEGORY_AND_ITEMS###'] = $this->cObj->substituteMarkerArrayCached($subpartArray['###ITEM_CATEGORY_AND_ITEMS###'],$quantitMarkerArray);
			}
