 ...
 * tracking functions
 *
 * $Id: class.tx_ttproducts_tracking.php 3625 2006-08-26 20:19:47Z franzholz $
 *
						...
						foreach ($langArray as $k => $v)	{
							if (($pos = strpos($k, $statusMessage))===0)	{
								...
								if (
									...
									t3lib_utility_Math::canBeInterpretedAsInteger($rest) :
									t3lib_div::testInt($rest)
								) {
	...
	*/
	function getTrackingInformation ($orderRow, $templateCode, $trackingCode, $updateCode, &$orderRecord, $admin) {
		...
		if (isset($orderRow) && is_array($orderRow) && $orderRow['uid'])	{
			$GLOBALS['TSFE']->page['title'] = $orderRow['uid'].' ('.$orderRow['bill_no'].'): '.$orderRow['name'].'-'.$orderRow['zip'].'-'.$orderRow['city'].'-'.$orderRow['country'];
