...
class tx_ttproducts_tracking_div {
	...
	function products_tracking(&$pibase, &$conf, &$config, &$basket, &$content, &$category, &$price, $theCode)	{
		...
		if (strcmp($theCode, 'TRACKING')!=0) { // bill and delivery tracking need more data
			...
			$this->page->initRecursive(999, $pibase);		// This add's all subpart ids to the pid_list based on the rootPid set in previous line
		...
		$admin = tx_ttproducts_tracking_div::shopAdmin($conf['update_code']);
		if (t3lib_div::_GP('tracking') || $admin)	{		// Tracking number must be set
			$orderRow = $basket->order->getOrderRecord('',t3lib_div::_GP('tracking'));
			if (is_array($orderRow) || $admin)	{		// If order is associated with tracking id.
				...
				switch ($theCode) {
					case 'TRACKING':
						$content = tx_ttproducts_tracking_div::getTrackingInformation($pibase,$conf,$basket, $basket->order, $orderRow,$pibase->templateCode);
						break;
					case 'BILL':
						$content = tx_ttproducts_billdelivery_div::getInformation($pibase,$conf,$config,$basket,$content,$category,$price,'bill',$orderRow, $pibase->templateCode,t3lib_div::_GP('tracking'));
						break;
					case 'DELIVERY':
						$content = tx_ttproducts_billdelivery_div::getInformation($pibase,$conf,$config,$basket,$content,$category,$price,'delivery',$orderRow, $pibase->templateCode,t3lib_div::_GP('tracking'));
						break;
					default:
						debug('error in '.TT_PRODUCTS_EXTkey.' calling function products_tracking with $theCode = "'.$theCode.'"');
				}
			} else {	// ... else output error page
				$content=$pibase->cObj->getSubpart($pibase->templateCode,tx_ttproducts_view_div::spMarker($pibase, $conf, '###TRACKING_WRONG_NUMBER###'));
				if (!$TSFE->beUserLogin)	{$content = $pibase->cObj->substituteSubpart($content,'###ADMIN_CONTROL###','');}
			}
		} else {	// No tracking number - show form with tracking number
			$content=$pibase->cObj->getSubpart($pibase->templateCode,tx_ttproducts_view_div::spMarker($pibase, $conf, '###TRACKING_ENTER_NUMBER###'));
			if (!$TSFE->beUserLogin)	{$content = $pibase->cObj->substituteSubpart($content,'###ADMIN_CONTROL###','');}
		}
		$markerArray=array();
		$markerArray['###FORM_URL###'] = $pibase->pi_getPageLink($TSFE->id,'',tx_ttproducts_view_div::getLinkParams()) ; // $this->getLinkUrl();	// Add FORM_URL to globalMarkerArray, linking to self.
		$content= $pibase->cObj->substituteMarkerArray($content, $markerArray);
	 ...
	 */
	function shopAdmin($update_code)	{
		$admin=0;
		if ($GLOBALS['TSFE']->beUserLogin)	{
			if (t3lib_div::_GP('update_code') == $update_code)	{
	 ...
	 */
	function getTrackingInformation(&$pibase,&$conf,&$basket, &$order, $orderRow, $templateCode)	{
			/*
		...
		if ($orderRow['uid'])	{
			...
			if (is_array($orderRecord['status']))	{
				...
				while(list(,$val)=each($orderRecord['status']))	{
					...
					if ($admin || ($val>=50 && $val<59))	{// Numbers 50-59 are usermessages.
						...
						tx_ttproducts_email_div::sendNotifyEmail($pibase, $conf, $order, $recipient, $status_log_element, t3lib_div::_GP('tracking'), $orderRow, $templateCode, $templateMarker);
						...
						while ($giftRow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($giftRes)) {
							...
							tx_ttproducts_email_div::sendGiftEmail($pibase, $conf, $basket, $recipient, $orderRecord['status_comment'], $giftRow, $templateCode, $templateMarker, $giftRow['personname'], $giftRow['personemail']);
						}
						$status_log[] = $status_log_element;
						$update=1;
					}
			...
			if (count($fieldsArray))	{		// If any items in the field array, save them
				...
				$orderRow = $order->getOrderRecord($orderRow['uid']);
			}
		}

			// Getting the template stuff and initialize order data.
		$content=$pibase->cObj->getSubpart($templateCode,'###TRACKING_DISPLAY_INFO###');
		...
		// added by Franz end
			...
			// Status:
		$STATUS_ITEM=$pibase->cObj->getSubpart($content,'###STATUS_ITEM###');
		$STATUS_ITEM_c='';
		if (is_array($status_log))	{
			reset($status_log);
			...
			while(list($k,$v)=each($status_log))	{
				...
				$STATUS_ITEM_c.=$pibase->cObj->substituteMarkerArrayCached($STATUS_ITEM, $markerArray);
			}
		}
		...
		if ($TSFE->beUserLogin)	{
				// Status admin:
			if (is_array($conf['statusCodes.']))	{
				reset($conf['statusCodes.']);
				while(list($k,$v)=each($conf['statusCodes.']))	{
					if ($k!=1)	{
		...
		$markerArray['###ORDER_UID###'] = $order->getOrderNumber($orderRow['uid']);
