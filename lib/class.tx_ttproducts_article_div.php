...
//	  $additional = $tmpArr[0]; 
	...
	function getVariantSubpartArray (&$pibase, &$tt_products, &$subpartArray, &$row, &$tempContent, $useSelects, &$conf)  {
		...
		tx_ttproducts_article_div::removeEmptySubpartArray($pibase, $tt_products, $subpartArray, $row, $conf);
	}
	...
	function removeEmptySubpartArray (&$pibase, &$tt_products, &$subpartArray, &$row, &$conf) {
		...
		$areaArray = array();
		if (trim($row['color']) == '' || !$conf['selectColor'])
			$areaArray[] = 'display_variant1';
		if (trim($row['size']) == '' || !$conf['selectSize'])
			$areaArray[] = 'display_variant2';
		if (trim($row['description']) == '' || !$conf['selectDescription'])
			$areaArray[] = 'display_variant3';
		if (trim($row['gradings']) == '' || !$conf['selectGradings'])
			$areaArray[] = 'display_variant4';
