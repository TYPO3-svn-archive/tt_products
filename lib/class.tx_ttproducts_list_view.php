...
// List products:
		...
		if ($theCode=='SEARCH')	{
				// Get search subpart
			$t['search'] = $this->pibase->cObj->getSubpart($templateCode,tx_ttproducts_view_div::spMarker('###ITEM_SEARCH###'));
		...
		if (($theCode!='SEARCH' && !t3lib_div::_GP('swords')) || $where)	{
			...
			if ($memoItems != '') {
				$t['listFrameWork'] = $this->pibase->cObj->getSubpart($templateCode,tx_ttproducts_view_div::spMarker('###MEMO_TEMPLATE###'));
			} else if ($theCode=='LISTGIFTS') {
				$t['listFrameWork'] = $this->pibase->cObj->getSubpart($templateCode,tx_ttproducts_view_div::spMarker('###ITEM_LIST_GIFTS_TEMPLATE###'));
			} else {
				$t['listFrameWork'] = $this->pibase->cObj->getSubpart($templateCode,tx_ttproducts_view_div::spMarker('###ITEM_LIST_TEMPLATE###'));
			}
...
/* Added els6: needed to display creditpoints in the credits category on the list page */
			...
			$content.=$this->pibase->cObj->getSubpart($templateCode,tx_ttproducts_view_div::spMarker('###ITEM_SEARCH_EMPTY###'));
		}
