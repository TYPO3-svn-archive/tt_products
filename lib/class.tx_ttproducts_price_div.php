...
class tx_ttproducts_price_div {
	...
	function getResellerPrice($row,$tax=1)	{
		...
		if ($priceNo > 0) {
			$returnPrice = tx_ttproducts_price_div::getPrice($row['price'.$priceNo],$tax,$row['tax']);
		}
		// normal price; if reseller price is zero then also the normal price applies
		if ($returnPrice == 0) {
			$returnPrice = tx_ttproducts_price_div::getPrice($row['price'],$tax,$row['tax']);
		}
		return $returnPrice;
	} // getResellerPrice
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tt_products/lib/class.tx_ttproducts_price_div.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tt_products/lib/class.tx_ttproducts_price_div.php']);
}
