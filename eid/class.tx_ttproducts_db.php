					...
					if ($uid)	{
						...
						if ($row = $TYPO3_DB->sql_fetch_assoc($res))	{
							...
							foreach ($priceTaxArray as $priceKey => $priceValue)	{
								...
								// $tmpRow[$fieldname] = $priceViewObj->printPrice($priceViewObj->priceFormat($priceValue, ''));
				...
				if (!in_array($field, $variantArray))	{
					...
					switch ($field)	{
						...
						case 'inStock':
							...
							$objResponse->addAssign('in-stock-id-'.$uid,'innerHTML',tx_div2007_alpha::getLL($langObj,($v > 0 ? 'in_stock' : 'not_in_stock')));
